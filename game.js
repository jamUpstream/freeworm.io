(() => {
    ...
    async function lbFetchTop(limit = 50) {
        if (!SUPABASE_URL || SUPABASE_URL === "YOUR_SUPABASE_PROJECT_URL") {
            throw new Error("Supabase not configured. Set SUPABASE_URL and SUPABASE_ANON_KEY in game.js.");
        }
        const url = `${SUPABASE_URL}/rest/v1/leaderboard?select=name,score,kills&order=score.desc&limit=${limit}`;
    ...
    async function lbSubmitScore(name, score, kills) {
        if (!SUPABASE_URL || SUPABASE_URL === "YOUR_SUPABASE_PROJECT_URL") {
            throw new Error("Supabase not configured. Set SUPABASE_URL and SUPABASE_ANON_KEY in game.js.");
        }
        ...
        const res = await fetch(url, {
            ...
            body: JSON.stringify({ name: String(name || ""), score: Math.round(score || 0), kills: Math.round(kills || 0) })
        });
...
`).join("");
    ...
    function showDeath() {
        ...
        if (saveScoreWrap) {
            if (!SUPABASE_URL || SUPABASE_URL === "YOUR_SUPABASE_PROJECT_URL") {
                saveScoreHint.textContent = "Leaderboard not configured. Set SUPABASE_URL and SUPABASE_ANON_KEY in game.js.";
                saveRow.style.display = "none";
                saveStatus.textContent = "";
            } else {
